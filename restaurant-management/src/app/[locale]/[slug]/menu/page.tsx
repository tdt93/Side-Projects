...
export default function PublicMenuPage() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/public/menu/${params.slug}${query ? `?${query}` : ""}`)
      .then((r) => r.json())
      .then(setData);
  }, [params.slug, locationId, tableId]);
    ...
    try {
      const res = await fetch("/api/public/orders", {
        ...
        body: JSON.stringify({
          ...
          fulfillmentType: data.table ? "dine-in" : fulfillmentType,
  ...
  if (!data) return <div className="flex min-h-screen items-center justify-center bg-background text-muted-foreground">Loading...</div>;
      ...
      <header className="sticky top-0 z-10 border-b border-border bg-card/95 px-4 py-4 backdrop-blur">
        ...
        {data.table && (
          <p className="text-xs text-muted-foreground">
            Table {data.table.number}{data.table.name ? ` - ${data.table.name}` : ""}
          </p>
        )}
      </header>
      <div className="flex gap-2 overflow-x-auto px-4 py-3">
        {categories.map((cat) => (
          <button key={cat} type="button" onClick={() => setCategory(cat)} className="shrink-0 rounded-full px-3 py-1.5 text-sm font-medium" style={{ background: category === cat ? data.tenant.primaryColor : "var(--muted)", color: category === cat ? "#fff" : undefined }}>
            {cat}
          </button>
        ))}
      </div>
        ...
        {filtered.map((item) => (
          <div key={item.id} className="overflow-hidden rounded-2xl border border-border bg-card">
            {item.imageUrl && (
              ...
              <img src={item.imageUrl} alt="" className={`w-full object-cover ${item.imageAspectRatio === "3:4" ? "aspect-[3/4]" : "aspect-square"}`} />
            )}
            <div className="p-4">
              <div className="flex items-start justify-between gap-2">
                <div><h3 className="font-semibold">{item.name}</h3><p className="text-xs text-muted-foreground">{item.description}</p></div>
                <p className="shrink-0 font-mono font-bold">{formatMoney(item.priceGrosze, data.currency)}</p>
              </div>
              <button type="button" onClick={() => addToCart(item)} className="mt-3 w-full rounded-xl py-2 text-sm font-semibold text-white" style={{ background: data.tenant.primaryColor }}>Add</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 border-t border-border bg-card p-4 shadow-lg">
          <div className="mx-auto flex max-w-lg items-center justify-between gap-4">
            <div><p className="text-sm text-muted-foreground">{cart.reduce((s, l) => s + l.quantity, 0)} items</p><p className="font-mono text-lg font-bold">{formatMoney(total, data.currency)}</p></div>
            <button type="button" onClick={() => setCheckoutOpen(true)} className="rounded-xl px-6 py-3 text-sm font-semibold text-white" style={{ background: data.tenant.primaryColor }}>Checkout</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-4 sm:items-center">
          <div className="w-full max-w-md rounded-2xl bg-card p-5">
              ...
              {cart.map((line) => (
                <div key={line.menuItemId} className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => changeQty(line.menuItemId, -1)} className="rounded bg-muted p-1"><Minus className="h-3 w-3" /></button>
                    <span className="font-mono">{line.quantity}</span>
                    <button type="button" onClick={() => changeQty(line.menuItemId, 1)} className="rounded bg-muted p-1"><Plus className="h-3 w-3" /></button>
                  </div>
                </div>
              ))}
            </div>
            <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {(["pickup", "delivery"] as const).map((type) => (
                    <button
                      ...
                      style={{
                        background: fulfillmentType === type ? data.tenant.primaryColor : "var(--muted)",
                        color: fulfillmentType === type ? "#fff" : undefined,
                      }}
                    >
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name *" className="w-full rounded-xl border border-border px-3 py-2 text-sm" />
              <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="Email" className="w-full rounded-xl border border-border px-3 py-2 text-sm" />
              <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone" className="w-full rounded-xl border border-border px-3 py-2 text-sm" />
            </div>
            <div className="mt-4 flex items-center justify-between">
              <span className="font-mono font-bold">{formatMoney(total, data.currency)}</span>
              <button type="button" disabled={submitting || !name.trim() || (!email.trim() && !phone.trim())} onClick={() => void submitOrder()} className="rounded-xl px-6 py-2 text-sm font-semibold text-white disabled:opacity-50" style={{ background: data.tenant.primaryColor }}>
                Place order
              </button>
            </div>
          </div>
