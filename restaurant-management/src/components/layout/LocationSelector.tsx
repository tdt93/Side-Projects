...
export function LocationSelector({ dark = false, compact = false }: { dark?: boolean; compact?: boolean }) {
  ...
  const baseClass = dark
    ? "rounded-lg border border-white/10 bg-white/5 text-[#FAFAF7]"
    : "rounded-lg border border-border bg-muted text-foreground";

  return (
    <div className={`flex items-center gap-1.5 ${compact ? "" : "min-w-0"}`}>
      {!compact && <MapPin className={`h-3.5 w-3.5 shrink-0 ${dark ? "text-[#6B5B50]" : "text-muted-foreground"}`} />}
      <select
        ...
        className={`max-w-[180px] truncate py-1.5 pl-2 pr-6 text-xs font-medium outline-none sm:max-w-[220px] ${baseClass}`}
        aria-label={t("selectBranch")}
      >
        {staffRole === "OWNER" && (
          <option value="__all__">{t("allLocations")}</option>
        )}
