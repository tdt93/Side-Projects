...
export async function generateTherapistProfileMetadata(
  slug: string,
): Promise<Metadata> {
...
export async function TherapistProfilePage({
  slug,
  searchParams,
}: {
  slug: string;
    ...
    <>
      ...
      <main className="flex-1">
        {(profile.officeCity || profile.officeAddressLine) && (
          <section className="border-b border-[#b9e9f5]/50 bg-[#BFE4EF]">
            <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-5 md:flex-row md:items-center md:justify-between">
              <h2 className="text-4xl font-bold tracking-tight text-[#163D8B] md:text-5xl">
                {profile.officeCity ?? "Gabinet"}
              </h2>
              <div className="text-left md:text-right">
                {profile.officeAddressLine ? (
                  <OfficeAddressLink className="inline-flex flex-col text-base leading-tight text-[#4F6E94] hover:text-[#163D8B]">
                    <span>{profile.officeAddressLine}</span>
                    <span className="text-sm text-[#2F7DB8] underline underline-offset-2">
                      (pokaż na mapie)
                    </span>
                  </OfficeAddressLink>
                ) : null}
        ...
        <WhyTrzymsieSection
          title={siteHome.whyTrzymsie.title}
          benefits={siteHome.whyTrzymsie.benefits}
        />
        ...
        <section id="recepcja" className="border-t border-[#b9e9f5]/50 bg-[#E4F4F8] py-14">
          <div className="mx-auto max-w-6xl px-4">
            ...
            <p className="mb-8 text-slate-600">
              Umów się na sesję psychoterapii lub zadaj nam pytanie dotyczące Twojego
              problemu/najbliższych wolnych terminów/dopasowania dla Ciebie odpowiedniego
              terapeuty lub procesu rezerwacji sesji, a skontaktujemy się z Tobą tak
              szybko, jak będzie to możliwe. Jeśli pragniesz, aby Twoje zapytanie zostało
              przekazane konkretnemu terapeucie, napisz, któremu.
            </p>
